...
public class MarksModelCell<T extends MarksModelGrid> extends AgentSQ2unstackable<T> {
    ...
    public void Mutate() {
        ...
        acidResistancePheno = BoundVal(acidResistancePheno + G().ACID_RESIST_MUTATION_RATE * delta, G().MAX_ACID_RESIST_PHENO, G().MIN_ACID_RESIST_PHENO);
        delta = Math.random() * 2.0 - 1.0;
        //mutate glycolysis pheno
        //System.out.print("BeforeGly: ")
        glycolysisPheno = BoundVal((double) (Math.pow(10, Math.log10(glycolysisPheno) + G().GLYCOLYSIS_MUTATION_RATE * delta)), G().MIN_GLYCOLYTIC_PHENO, G().MAX_GLYCOLYTIC_PHENO);
    ...
    public int CheckDivide() {
        int iMoveOpt = 0;
        int nCheck = G().SQsToLocalIs(G().MOORE_NEIGHBORHOOD, G().moveIs, Xsq(), Ysq(), false, false);
    ...
    public <Q extends MarksModelCell> Q Divide(int iDiv) {
        Q child = (Q) G().NewAgent(iDiv);
    ...
    public boolean CellDeathSkipStep(){
        if (IsAlive()) {
            if (!IsVessel()) {
                ...
                double pH = ConvertHToPh(G().protons.GetCurr(Isq()));
                //check acid death
    ...
    public void SetRates() {
        ...
        if (IsAlive()) {
            ...
            if (!IsVessel()) {
                //cell consumption
                double o2Rate = -MichaelisMenten(G().oxygen.GetCurr(i), G().OXYGEN_MAX_RATE, G().OXYGEN_HALF_RATE_CONC);
                ...
                double gluRate = -Utils.MichaelisMenten(G().glucose.GetCurr(i), maxGlycolysisRate, G().GLUCOSE_HALF_RATE_CONC);
                ...
                G().protons.SetCurr(i, G().protons.GetCurr(i) + protonRate);
                G().glucose.SetCurr(i, G().glucose.GetCurr(i) + gluRate);
                G().oxygen.SetCurr(i, G().oxygen.GetCurr(i) + o2Rate);
            } else {
                //vessel production
                G().glucose.SetCurr(i, G().GLUCOSE_VESSEL_CONC);
                G().oxygen.SetCurr(i, G().OXYGEN_VESSEL_CONC);
                G().protons.SetCurr(i, G().ACID_VESSEL_CONC);
            }
        }
    }
