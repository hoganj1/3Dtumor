...
public class Binomial {
    ...
    public Binomial(Random randomGenerator) {
    ...
    public long generateBinomial(long n, double p) {
        ...
        if(this.np < 10.0D) {
            ...
            while(true) {
                ...
                while(true) {
                    ...
                    if((U = this.randomGenerator.nextDouble() * this.p4) <= this.p1) {
    ...
    private void setNandP(int n, double p) {
        if((double)n * Math.min(p, 1.0D - p) <= 0.0D) {
            throw new IllegalArgumentException();
        } else {
            this.n = n;
            this.p = p;
            this.log_p = Math.log(p);
            this.log_q = Math.log(1.0D - p);
            this.log_n = logFactorial(n);
        }
    }
}
